...
public class SpPrepareTest {
	...
	public static String testSubquery(String select_list, String from, String where, String value) {
		...
		Class.forName("cubrid.jdbc.driver.CUBRIDDriver");
		Connection conn = DriverManager.getConnection("jdbc:default:connection:","","");
		String query = String.format("select %s from %s where %s = ?", select_list, from, where);
	 ...
	 * SELECT h.host_year, testSubquery('o.host_nation', 'olympic o', 'o.host_year', h.host_year) AS host_nation, h.event_code, h.score, h.unit FROM history2 h LIMIT 20
	 */
	public static String testSubquery2(String select_list, String from, String where, String value) {
		try {
		Class.forName("cubrid.jdbc.driver.CUBRIDDriver");
		Connection conn = DriverManager.getConnection("jdbc:default:connection:","","");
		String query = String.format("select %s from %s where %s = ?", select_list, from, where);
	...
	// CREATE OR REPLACE FUNCTION JSON_QUERY_LINK(q STRING) RETURN STRING AS LANGUAGE JAVA NAME 'SpPrepareTest.json_link(java.lang.String) return java.lang.String';
		...
		try {
			...
			while (rs.next()) {
				...
				for (int i = 1; i < numColumns + 1; i++) {
					...
					if (rsmd.getColumnType(i) == java.sql.Types.ARRAY) {
						builder.append("\"" + column_name + "\" :" + rs.getArray(column_name) + ",");
						...
						builder.append("\"" + column_name + "\" :" + rs.getFloat(column_name) + ",");
						//obj.put(column_name, rs.getFloat(column_name));
					} else if (rsmd.getColumnType(i) == java.sql.Types.NVARCHAR) {
						builder.append("\"" + column_name + "\" :" + rs.getNString(column_name) + ",");
						//obj.put(column_name, rs.getNString(column_name));
					} else if (rsmd.getColumnType(i) == java.sql.Types.VARCHAR) {
						builder.append("\"" + column_name + "\" : \"" + rs.getString(column_name) + "\",");
