...
class hnsw_usearch_ng final:public hnsw_index
{
  ...
  protected:
    ...
    std::mutex m_build_worker_job_queue_mutex;
    std::condition_variable m_build_worker_job_queue_cv;
    bool m_build_worker_job_queue_ready {false};
...
hnsw_usearch_ng::add (cubthread::entry *thread_p, int n_vectors, const OID *oid, const float *vector)
{
  ...
  for (int i = 0; i < n_vectors; ++i)
	...
	{
	  ctx.m_pushed_tasks++;
	  m_build_worker_job_queue->produce (job);
	}
      else
	{
