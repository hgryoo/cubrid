			  ...
			  std::wregex **comp_regex, char **comp_pattern)
{
  ...
  std::wregex *rx_compiled_regex = NULL;
  char *rx_compiled_pattern = NULL;
  DB_LOGICAL result_value = V_FALSE;
  {
  ...
  try
    {
      ...
      {
        bool match = std::regex_search (wsrc, *rx_compiled_regex);
        result_value = match ? V_TRUE : V_FALSE;
      }
      else
      {
        result_value = V_FALSE;
			  ...
			  std::wregex **comp_regex, char **comp_pattern)
{
...

  {
    ...
    const DB_VALUE *position = NULL;
    const DB_VALUE *occurrence = NULL;
    const DB_VALUE *return_opt = NULL;
    const DB_VALUE *match_type = NULL;
    ...
    if (num_args >= 5)
      {
	return_opt = args[4];
	assert (return_opt != (DB_VALUE *) NULL);
	is_any_null |= DB_IS_NULL (return_opt);
      }
    ...
    if (occurrence_value >= 0)
	  {
      int return_opt_value = 0;
      if (return_opt)
      {
        return_opt_value = db_get_int (return_opt);
      }
