...
//static int xts_save_string (const std::string_view str);
...
static char *xts_process (char *ptr, const struct method_sig_node & sig);
static char *xts_process (char *ptr, const struct method_sig_list & sig_list);
static char *xts_process (char *ptr, const struct cubxasl::method_spec_node & spec);
...
static int xts_sizeof (const struct method_sig_node& ptr);
static int xts_sizeof (const struct method_sig_list& ptr);
static int xts_sizeof (const struct cubxasl::method_spec_node& ptr);
...
/*
static int
xts_save_string (const std::string_view string)
{
  return xts_save_string (string.data());
}
*/
...
xts_process_access_spec_type (char *ptr, const ACCESS_SPEC_TYPE * access_spec)
{
  ...
  switch (access_spec->type)
    ...
    case TARGET_METHOD:
      ptr = xts_process (ptr, ACCESS_SPEC_METHOD_SPEC (access_spec));
      break;
...
static char*
xts_process (char *ptr, const struct method_sig_node & sig)
{
  int offset;
  char *start_ptr = ptr;
  int n;

  offset = xts_save_string (sig.method_name.c_str ());
  if (offset == ER_FAILED)
    {
      return NULL;
    }
  ...
  ptr = or_pack_int (ptr, sig.method_type);
  
  ptr = or_pack_int (ptr, sig.num_method_args);

  int num_args = sig.num_method_args;
  for (n = 0; n < num_args + 1; n++)
    {
      ptr = or_pack_int (ptr, sig.method_arg_pos[n]);
    }

  offset = xts_save_string (sig.class_name.c_str ());	/* is can be null */
  if (offset == ER_FAILED)
     {
            return NULL;
     }
  ...
  for (n = 0; n < num_args; n++)
    {
      ptr = or_pack_int (ptr, sig.arg_info.arg_mode[n]);
    }
  for (n = 0; n < num_args; n++)
    {
      ptr = or_pack_int (ptr, sig.arg_info.arg_type[n]);
    }

  ptr = or_pack_int (ptr, sig.arg_info.result_type);

  xts_debug_check (sig, start_ptr, ptr);
 ...
 */
static char *
xts_process (char *ptr, const struct method_sig_list & method_sig_list)
{
  char *start_ptr = ptr;

  int num_methods = (int) method_sig_list.method_sigs.size ();
  ptr = or_pack_int (ptr, num_methods);

  for (int i = 0; i < num_methods; i++)
  {
          ptr = xts_process (ptr, method_sig_list.method_sigs[i]); /* method_sig_nodes */
  }

  xts_debug_check (method_sig_list, start_ptr, ptr);

  return ptr;
}

static char *
xts_process (char *ptr, const struct cubxasl::method_spec_node & method_spec)
{
  int offset;
  char *start_ptr = ptr;
  ...
  offset = xts_save_xasl_node (method_spec.xasl);
  if (offset == ER_FAILED)
    {
      return NULL;
    }
  ...
  offset = xts_save (*method_spec.method_sig_list);
  if (offset == ER_FAILED)
    {
      return NULL;
    }
  ...
  // ptr = xts_process (ptr, *method_spec.method_sig_list);

  xts_debug_check (method_spec, start_ptr, ptr);
...
xts_sizeof_access_spec_type (const ACCESS_SPEC_TYPE * access_spec)
{
  ...
  switch (access_spec->type)
    ...
    case TARGET_METHOD:
      tmp_size = xts_sizeof (ACCESS_SPEC_METHOD_SPEC (access_spec));
      if (tmp_size == ER_FAILED)
	{
	  return ER_FAILED;
	}
...
static int
xts_sizeof (const struct method_sig_node & method_sig)
{
  int size = 0;
  
  size += PTR_SIZE;	/* method_name (std::string) */
  
  size += OR_INT_SIZE;	/* method_type */
  
  size += OR_INT_SIZE;	/* num_method_args */

  size += OR_INT_SIZE * (method_sig.num_method_args + 1); /* method_arg_pos */

  size += PTR_SIZE;	/* class_name */

  size += (OR_INT_SIZE * method_sig.num_method_args);	/* arg_mode */

  size += (OR_INT_SIZE * method_sig.num_method_args);	/* arg_type */
 ...
 */
static int
xts_sizeof (const struct method_sig_list & sig_list)
{
  int size = 0;

  size += OR_INT_SIZE;		/* num_methods */

  for (const struct method_sig_node& n : sig_list.method_sigs)
  {
        size += xts_sizeof (n);        /* method_sig_nodes */
  }
 ...
 */
static int
xts_sizeof (const struct cubxasl::method_spec_node & method_spec)
{
  int size = 0;
...
/*
 * xts_sizeof () - template function to pack std::vector structur into XASL stream
 *   return:
 *   ptr(in)    :
 */
template <typename T>
static int
xts_sizeof (const std::vector<T> & vec, bool is_include_size)
{
  int size = 0;
