...
parse_default_expr_type(const char *str, const int str_size, int *next_len) {
    ...
    switch (str[0]) {
        case 'S':
            if (str_size >= 8) {
                if (strncmp(str, "SYS_DATE", 8) == 0) { *next_len = 8; return DB_DEFAULT_SYSDATE; }
                if (strncmp(str, "SYS_TIME", 8) == 0) { *next_len = 8; return DB_DEFAULT_SYSTIME; }
            }
            if (str_size >= 11 && strncmp(str, "SYS_DATETIME", 11) == 0) { *next_len = 11; return DB_DEFAULT_SYSDATETIME; }
            if (str_size >= 12 && strncmp(str, "SYS_TIMESTAMP", 12) == 0) { *next_len = 12; return DB_DEFAULT_SYSTIMESTAMP; }
            break;

        case 'C':
            if (str_size >= 12) {
                if (strncmp(str, "CURRENT_DATE", 12) == 0) { *next_len = 12; return DB_DEFAULT_CURRENTDATE; }
                if (strncmp(str, "CURRENT_TIME", 12) == 0) { *next_len = 12; return DB_DEFAULT_CURRENTTIME; }
            }
            if (str_size >= 16 && strncmp(str, "CURRENT_DATETIME", 16) == 0) { *next_len = 16; return DB_DEFAULT_CURRENTDATETIME; }
            if (str_size >= 17 && strncmp(str, "CURRENT_TIMESTAMP", 17) == 0) { *next_len = 17; return DB_DEFAULT_CURRENTTIMESTAMP; }
            break;

        case 'U':
            if (str_size >= 15 && strncmp(str, "UNIX_TIMESTAMP()", 15) == 0) { *next_len = 15; return DB_DEFAULT_UNIX_TIMESTAMP; }
            if (str_size >= 6 && strncmp(str, "USER()", 6) == 0) { *next_len = 6; return DB_DEFAULT_USER; }
            if (str_size >= 4 && strncmp(str, "USER", 4) == 0) { *next_len = 4; return DB_DEFAULT_USER; }
            break;
    }
