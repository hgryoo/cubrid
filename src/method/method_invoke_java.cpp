...
namespace cubmethod
{
  ...
  {
    UINT64 s_id = group->get_thread_entry ()->conn_entry->session_id;
    m_header = new cubmethod::header (s_id, METHOD_REQUEST_CALLBACK /* default */, -1 /* dummy */);
  }
  ...
  int method_invoke_java::invoke (cubthread::entry *thread_p, std::vector<std::reference_wrapper<DB_VALUE>> &arg_base)
  {
    ...
    cubmethod::header header (s_id, SP_CODE_INVOKE /* default */, m_group->get_and_increment_request_id());
  ...
  method_invoke_java::callback_get_db_parameter (cubthread::entry &thread_ref, packing_unpacker &unpacker)
  {
    ...
    UINT64 s_id = m_group->get_thread_entry ()->conn_entry->session_id;
    cubmethod::header header (s_id, SP_CODE_INTERNAL_JDBC /* default */, m_group->get_and_increment_request_id());
  ...
  method_invoke_java::callback_prepare (cubthread::entry &thread_ref, packing_unpacker &unpacker)
  {
      ...
      if (res_code == METHOD_RESPONSE_SUCCESS)
	{
      ...
      UINT64 s_id = m_group->get_thread_entry ()->conn_entry->session_id;
      cubmethod::header header (s_id, SP_CODE_INTERNAL_JDBC /* default */, m_group->get_and_increment_request_id());
      error = mcon_send_data_to_java (m_group->get_socket (), header, b);
      return error;
    };
